...
function get_relative_url() {
    ...
    reh_debug("URL: Relative URL is: " + relative_url);
...
if ( getURLvar("debug") && ( getURLvar("debug") == "true" || getURLvar("debug") == "1" ) ) {
    var reh_debug_config = true;
    reh_debug("Debug: URL debug variable enabled");
} else {
    var reh_debug_config = 0;
    reh_debug("Debug: skin.conf reh_debug enabled");
}
...
jQuery(document).ready(function() {
    ...
    if ( sessionStorage.getItem('theme') == "toggleOverride" ) {
        reh_debug("Theme: sessionStorage override in place.");
    ...
    if ( window.location.search.indexOf('theme') ) {
        if (window.location.search.indexOf('?theme=dark') === 0) {
            reh_debug("Theme: Setting dark theme because of URL override");
            changeTheme("dark", true);
        } else if (window.location.search.indexOf('?theme=light') === 0) {
            reh_debug("Theme: Setting light theme because of URL override");
            changeTheme("light", true);
        } else if (window.location.search.indexOf('?theme=auto') === 0) {
            reh_debug("Theme: Setting auto theme because of URL override");
            sessionStorage.setItem('theme', 'auto')
            autoTheme(16, 26, 07, 55)
        }
    }
    ...
    try {
        document.getElementById('themeSwitch').addEventListener('change', function(event) {
            reh_debug("Theme: Toggle button changed");
...
function rotateThis(newRotation) {
    ...
    reh_debug("rotateThis: rotating to " + newRotation);
    var currentRotation;
...
function autoTheme(sunset_hour, sunset_min, sunrise_hour, sunrise_min) {
    ...
    if ( getURLvar("theme") && getURLvar("theme") != "auto" ) {
        reh_debug("Auto theme: theme override detected in URL. Skipping auto theme");
        return true;
    }
    reh_debug("Auto theme: checking to see if theme needs to be switched");
    ...
    reh_debug("Auto theme: sunrise: " + sunrise_hour);
    reh_debug("Auto theme: now: " + nowHour);
    reh_debug("Auto theme: sunset: " + sunset_hour);
    reh_debug("Auto theme: are we in daylight hours: " + dayTime);
    reh_debug("Auto theme: sessionStorage.getItem('theme') = " + sessionStorage.getItem('theme'));
    
    if ( dayTime ) {
        // Day time, set light if needed
        if ( document.body.classList.contains("dark") ) {
            if ( sessionStorage.getItem('theme') == "auto" ) {
                reh_debug("Auto theme: setting light theme since dayTime variable is true (day)");
            } else {
                reh_debug("Auto theme: cannot set light theme since visitor used toggle to override theme. Refresh to reset the override.");
            }
        ...
        if ( document.body.classList.contains("light") ) {
            if ( sessionStorage.getItem('theme') == "auto" ) {
                reh_debug("Auto theme: setting dark theme since dayTime variable is false (night)");
            } else {
                reh_debug("Auto theme: cannot set dark theme since visitor used toggle to override theme. Refresh to reset the override.");
            }
            // Only change theme if user has not overridden the auto option with the toggle
            if ( sessionStorage.getItem('theme') == "auto" ) {
                changeTheme("dark");
...
function changeTheme(themeName, toggleOverride=false) {
    reh_debug("Theme: Changing to " + themeName);
    // If the configured theme is auto, but the user toggles light/dark, remove the auto option.
    if ( toggleOverride ) {
        reh_debug("Theme: toggle override clicked.");
        reh_debug("Theme: sessionStorage.getItem('theme') was previously: " + sessionStorage.getItem('theme') );
        // This was applied only to auto theme config, but now it's applied to all themes so visitor has full control on light/dark mode
        //if ( sessionStorage.getItem('theme') == "auto" ) { }
        sessionStorage.setItem('theme', 'toggleOverride');
        reh_debug("Theme: sessionStorage.getItem('theme') is now: " + sessionStorage.getItem('theme') );
    }
    if ( themeName == "dark" ) {
        // Apply dark theme
...
function update_weewx_data( data ) {
    ...
    jQuery(".snapshot-records-today-header").html( moment.unix( data["current"]["epoch"] ).utcOffset(-480).format( 'dddd, LL' ) );
    jQuery(".snapshot-records-month-header").html( moment.unix( data["current"]["epoch"] ).utcOffset(-480).format( 'MMMM YYYY' ) );
    ...
    // Sunrise and Sunset            
    jQuery(".sunrise-value").html( moment.unix( parseFloat(data["almanac"]["sunrise_epoch"]).toFixed(0) ).utcOffset(-480).format( "LT" ) );
    jQuery(".sunset-value").html( moment.unix( parseFloat(data["almanac"]["sunset_epoch"]).toFixed(0) ).utcOffset(-480).format( "LT" ) );
    ...
    almanac_updated = "Last Updated " + moment.unix( data["current"]["datetime_raw"] ).utcOffset( -480 ).format( "LL, LTS" );
...
function show_forcast_alert( data, forecast_provider ) {
    reh_debug("Forecast: Updating alert data for " + forecast_provider);
    ...
    if ( forecast_provider == "darksky" ) {
        if ( data['alerts'] ) {
            for ( i = 0; i < data['alerts'].length; i++ ) { 
                ...
                forecast_alert_expires = moment.unix( data['alerts'][i]['expires'] ).utcOffset(-480).format( 'LLL' );
        ...
        if ( data['alerts'][0]['response'][0] ) {
            for ( i = 0; i < data['alerts'][0]['response'].length; i++ ) {             
                ...
                forecast_alert_expires = moment.unix( data['alerts'][0]['response'][i]['timestamps']['expires'] ).utcOffset(-480).format( 'LLL' );
                forecast_alerts.push( { "title": forecast_alert_title, "body": forecast_alert_body, "link": forecast_alert_link, "expires": forecast_alert_expires });
            }
        }
    }
    ...
    if ( forecast_alerts.length > 0 ) {
        reh_debug("Forecast: There are " + forecast_alerts.length + " alert(s).");
        ...
        reh_debug("Forecast: There are no forecast alerts");
...
function update_forecast_data( data ) {
    ...
    reh_debug("Forecast: Provider is " + forecast_provider);
    reh_debug("Forecast: Updating data");
    
    forecast_row = [];
    
    if ( forecast_provider == "N/A" ) {
        jQuery(".forecastrow").hide();
        reh_debug("Forecast: No provider, hiding forecastrow");
        return;
    } else if ( forecast_provider == "darksky" ) {
        
        var forecast_subtitle = moment.unix( data["currently"]["time"] ).utcOffset(-480).format( 'LLL' );
        ...
        reh_debug("Forecast: icon is " + wxicon);
        ...
        for (i = 0; i < 7; i++) {
            ...
            var weekday = moment.unix( data["daily"]["data"][i]["time"] + 7200 ).utcOffset(-480).format( "ddd M/DD" );
            ...
            var forecast_link = '<a href="https://darksky.net/details/'+data["latitude"]+','+data["longitude"]+'/'+moment.unix( data["daily"]["data"][i]["time"] + 7200 ).utcOffset(-480).format( "YYYY-M-D" )+'/'+darksky_link_units+'/" target="_blank">Daily Forecast</a>';
    ...
    if ( forecast_provider == "aeris" ) {
        var forecast_subtitle = moment.unix( data["timestamp"] ).utcOffset(-480).format( 'LLL' );
        
        try {
            reh_debug("Forecast: icon from Aeris data is " + data["current"][0]["response"]["ob"]["icon"] );
            var wxicon = get_relative_url() + "/images/" + aeris_icon( data["current"][0]["response"]["ob"]["icon"] ) + ".png";
            reh_debug("Forecast: reh icon is " + wxicon);
            // Current observation text
        ...
        for (i = 0; i < data["forecast"][0]["response"][0]["periods"].length; i++) {
            ...
            var weekday = moment.unix( data["forecast"][0]["response"][0]["periods"][i]["timestamp"] + 7200 ).utcOffset(-480).format( "ddd M/DD" );
            ...
            var forecast_link_setup = "".replace("YYYY", moment.unix( data["forecast"][0]["response"][0]["periods"][i]["timestamp"] + 7200 ).utcOffset(-480).format( "YYYY" )).replace("MM", moment.unix( data["forecast"][0]["response"][0]["periods"][i]["timestamp"] + 7200 ).utcOffset(-480).format( "MM" )).replace("DD", moment.unix( data["forecast"][0]["response"][0]["periods"][i]["timestamp"] + 7200 ).utcOffset(-480).format( "DD" ));
    ...
    reh_debug("Forecast: Changing icon to " + wxicon);
...
function showChart(json_file, prepend_renderTo=false) {
    ...
    jQuery.getJSON(get_relative_url() + '/json/' + json_file + '.json', function(data) {
        ...
        jQuery.each(data, function (plotname, obsname) {
            ...
            // Ignore the reh Version since this "plot" has no other options
            if ( plotname == "reh_version" ) {
                return true;
            }
                ...
                tooltip: {
                    ...
                    formatter: function (tooltip) {
                                    try {
                                        ...
                                        return [moment.unix( this.x / 1000).utcOffset(-480).format( tooltip_date_format )].concat(
                                        ...
                                        if (this.series.userOptions.obsType == "windDir" || this.series.userOptions.observation_type == "windDir") {
                                            ...
                                            return moment.unix( this.x / 1000).utcOffset(-480).format( tooltip_date_format ) +'<br><b>' + highcharts_tooltip_factory( this.point.y, point_obsType, true, rounding, mirrored );
            ...
            reh_debug( options.chart.renderTo + ": building a " + type + " chart" );
            
            if ( css_class ) {
                jQuery( "#" + options.chart.renderTo ).addClass( css_class );
                reh_debug( options.chart.renderTo + ": div id is " + options.chart.renderTo + " and adding CSS class: " + css_class );
            }
            ...
            if ( xAxis_categories.length >= 1 ) {
                reh_debug( options.chart.renderTo + ": has " + xAxis_categories.length + " xAxis categories. Resetting xAxis and tooltips for grouping" );
                options.xAxis = {}
            ...
            */
            ...
            options.series.forEach(s => {
                ...
                reh_debug( options.chart.renderTo + ": " + s.obsType + " is on yAxis " + this_yAxis );
                ...
                // Rain, RainRate and rainTotal (special reh skin observation) get yAxis precision
                ...
                if ( s.mirrored_value ) {
                    reh_debug( options.chart.renderTo + ": mirrored chart due to mirrored_value = true" );
            ...
            if (observation_type == "weatherRange") {
                ...
                if ( JSON.parse( String( options.series[0].polar.toLowerCase() ) ) ) {
                    ...
                    options.chart.className = "highcharts-weatherRange reh-polar"; // Used for dark mode
                } else {
            ...
            reh_debug("Highcharts.chart('container', " + JSON.stringify(options) + ");");
