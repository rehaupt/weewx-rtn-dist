...
if weewx.__version__ < "4":
    ...
    def logmsg(level, msg):
        syslog.syslog(level, "reh Extension: %s" % msg)
...
aqi_time = 0
aqi_location = ""
aqi_purpleair_station_id = ""


class rehData(SearchList):
    """
    ...
    def get_cardinal_direction(self, degree, return_only_labels=False):
        default_ordinate_names = ['N', 'NNE', 'NE', 'ENE', 'E', 'ESE', 'SE',
                                  'SSE', 'S', 'SSW', 'SW', 'WSW', 'W', 'WNW', 'NW', 'NNW', 'N/A']
        try:
    ...
    def get_extension_list(self, timespan, db_lookup):
        """
        Build the data needed for the reh skin
        """
        ...
        global aqi_time
        global aqi_location
        global aqi_purpleair_station_id
        ...
        reh_debug = self.generator.skin_dict['Extras'].get('reh_debug', 0)
        ...
        moment_js_utc_offset = (calendar.timegm(moment_js_stop_struct) -
                                calendar.timegm(time.gmtime(time.mktime(moment_js_stop_struct)))) / 60
       ...
       # Highcharts UTC offset is the opposite of normal. Positive values are
        ...
        if self.generator.skin_dict['Extras']['reh_locale'] == "auto":
            ...
            try:
               # Try setting the locale. Locale needs to be in locale.encoding
                ...
                locale.setlocale(
                    locale.LC_ALL,
                    self.generator.skin_dict['Extras']['reh_locale']
                )
                ...
                if reh_debug:
                    logerr(
                        ...
                        "reh skin JavaScript will try to use this locale. Full error: %s"
                        % (
                            self.generator.skin_dict["Extras"]["reh_locale"],
                            error,
                        )
        ...
        try:
            ...
            system_locale_js = system_locale.replace("_", "-")
        except:
        ...
        try:
            archive_interval_ms = (
                int(self.generator.config_dict["StdArchive"]
        ...
        if os.path.exists(chart_config_path):
            chart_dict = configobj.ConfigObj(
                chart_config_path, file_error=True)
        else:
            chart_dict = configobj.ConfigObj(
        ...
        for gg in graph_page_graphgroup_buttons:
            if "button_text" in chart_dict[gg]:
                ...
                button_text = gg
            graph_page_buttons += (
                '<a href="./?graph='+gg+'"><button type="button" class="btn btn-primary">' + button_text + '</button></a>')
        ...
        # there with reh 0.1 through 0.7 are showing the visitor's
        ...
        if self.generator.skin_dict["Extras"].get("radar_html") == "":
            if self.generator.skin_dict["Extras"].get("aeris_map") == "1":
                radar_html = '<img style="object-fit:cover;width:{}px;height:{}px" src="https://maps.aerisapi.com/{}_{}/flat,water-depth,counties:60,rivers,interstates:60,admin-cities,alerts-severe:50:blend(darken),radar:blend(darken)/{}x{}/{},{},{}/current.png"></img>'.format(
                    radar_width,
                    radar_height,
                    self.generator.skin_dict["Extras"]["aeris_forecast_api_id"],
                    self.generator.skin_dict["Extras"]["aeris_forecast_api_secret"],
                    radar_width,
                    radar_height,
                    lat,
                    lon,
        ...
        if year_outTemp_max_range_query is not None:
            ...
            year_outTemp_max_range_total = outTemp_round % (
                float(year_outTemp_max_range_max) -
                float(year_outTemp_max_range_min)
            )
        ...
        if year_outTemp_min_range_query is not None:
            ...
            year_outTemp_min_range_total = outTemp_round % (
                float(year_outTemp_min_range_max) -
                float(year_outTemp_min_range_min)
            )
            # Replace the SQL Query output with the converted values
        ...
        if rainiest_day_query is not None:
            rainiest_day_tuple = (
                rainiest_day_query[1], rain_unit, "group_rain")
            rainiest_day_converted = (
                rain_round % self.generator.converter.convert(rainiest_day_tuple)[
                    0]
            )
            rainiest_day = [
            ...
            rainiest_day = [calendar.timegm(
                time.gmtime()), locale.format("%.2f", 0)]
        ...
        at_rainiest_day_tuple = (
            at_rainiest_day_query[1], rain_unit, "group_rain")
        at_rainiest_day_converted = (
            rain_round % self.generator.converter.convert(
                at_rainiest_day_tuple)[0]
        )
        at_rainiest_day = [
        ...
        at_rainiest_month_tuple = (
            at_rainiest_month_query[2], rain_unit, "group_rain")
        at_rainiest_month_converted = (
            rain_round % self.generator.converter.convert(
                at_rainiest_month_tuple)[0]
        )
        at_rainiest_month_name = calendar.month_name[int(
            at_rainiest_month_query[0])]
        at_rainiest_month = [
        ...
        at_rain_highest_year_query = wx_manager.getSql(
        ...
        for row in year_rain_query:
            ...
            year_days_without_rain_output[row[0]
                                          ] = year_days_without_rain_total
        ...
        try:
            ...
            for y in years:
                ...
                if os.path.exists(noaa_dir + "NOAA-%s.txt" % y):
                    noaa_header_html += (
                        '<a href=\"?yr=%s\" class=\"noaa_rep_nav\"><b>%s</b></a>:' % (
                            y, y)
                    )
                else:
                    noaa_header_html += (
                ...
                noaa_header_html += "<br>"
                # loginf("noaa_html: %s" % (json.dumps(noaa_header_html)))
            ...
            if os.path.exists(
                ...
                default_noaa_file = "NOAA-%s-%s.txt" % (
            ...
            loginf("NOAA File Exception **  noaa_html: %s" % (json.dumps(noaa_header_html)))
            pass
        
        if (self.generator.skin_dict["Extras"]["aqi_enabled"] == "1" and self.generator.skin_dict["Extras"]["aqi_purpleair_station_id"] != ""): 
            aqi_purpleair_station_id = self.generator.skin_dict["Extras"]["aqi_purpleair_station_id"]
        ...
        if (
            ...
            and self.generator.skin_dict["Extras"]["aeris_forecast_api_id"] != ""
            ...
            aeris_forecast_api_id = self.generator.skin_dict["Extras"]["aeris_forecast_api_id"]
            aeris_forecast_api_secret = self.generator.skin_dict["Extras"][
                "aeris_forecast_api_secret"
            ]
            darksky_forecast_api_secret = self.generator.skin_dict["Extras"][
                "darksky_forecast_api_secret"
            ]
            
            meteoblue_api_key = "l3h4Okip2ICdausS"
            
            forecast_units = self.generator.skin_dict["Extras"][
                "forecast_units"
            ].lower()
            
            forecast_nws_file = html_root + "/json/forecast_nws.json"
            forecast_nws_url = "https://api.weather.gov/gridpoints/SEW/130,59/forecast"
            forecast_nws_is_stale = False
            
            normals_file = html_root + "/json/normals.json"
            normals_is_stale = False
            
            normals_stale_timer = self.generator.skin_dict["Extras"][
                "normals_stale"
            ]
            ...
            if forecast_provider == "aeris":
                if self.generator.skin_dict["Extras"]["forecast_aeris_use_metar"] == "1":
                    forecast_current_url = (
                        ...
                        % (latitude, longitude, aeris_forecast_api_id, aeris_forecast_api_secret)
                    )
                else:
                    # forecast_current_url = (
                    #     "https://api.aerisapi.com/observations/%s,%s?&format=json&filter=allstations&limit=1&client_id=%s&client_secret=%s"
                    #     % (latitude, longitude, aeris_forecast_api_id, forecast_api_secret))
                    forecast_current_url = (
                        "https://api.aerisapi.com/conditions/%s,%s?format=json&plimit=1&filter=1min&client_id=%s&client_secret=%s" % (latitude, longitude, aeris_forecast_api_id, aeris_forecast_api_secret))
                # metar_current_url = ("https://api.aerisapi.com/observations/search?query=id:KSEA&format=json&client_id=%s&client_secret=%s" % (aeris_forecast_api_id, aeris_forecast_api_secret))
                metar_current_url = ("https://api.aerisapi.com/observations/%s,%s?&format=json&filter=allstations&filter=metar&limit=1&client_id=%s&client_secret=%s" %
                                    (latitude, longitude, aeris_forecast_api_id, aeris_forecast_api_secret))
                forecast_24hr_url = (
                ...
                aqi_purpleair_url = ("https://api.purpleair.com/v1/sensors/%s" % (aqi_purpleair_station_id))

                 # Fetch normal temperatures (high/low) for today
                temp_normals_url = "https://api.aerisapi.com/normals/%s,%s?format=json&client_id=%s&client_secret=%s" % (
                    latitude,
                    longitude,
                    aeris_forecast_api_id,
                    aeris_forecast_api_secret,
                )
            ...
            elif forecast_provider == "meteoblue":
                if self.generator.skin_dict["Extras"]["forecast_alert_limit"]:
                    forecast_alert_limit = self.generator.skin_dict["Extras"][
                        "forecast_alert_limit"
                    ]
                forecast_url = "https://my.meteoblue.com/packages/basic-3h_basic-day_current_clouds-3h?apikey=%s&lat=%s&lon=%s&asl=299&format=json&temperature=F&windspeed=mph&precipitationamount=inch&forecast_days=7" % ( meteoblue_api_key, latitude, longitude )
                

                metar_current_url = ("https://api.aerisapi.com/observations/search?query=id:KPSP&format=json&client_id=%s&client_secret=%s" % (aeris_forecast_api_id, aeris_forecast_api_secret))
            ...
            # Determine if the Normals File Exist -- Only need it once a day
            if os.path.isfile(normals_file):
                with open(normals_file, "r") as read_file:
                    data = json.load(read_file)
                if (int(time.time()) - int(os.path.getmtime(normals_file))) > int(
                    normals_stale_timer
                ):
                    normals_is_stale = True
            else :
                normals_is_stale = True
            ...
            if normals_is_stale :
                ...
                print ("normals is stale")
                req = Request(temp_normals_url, None, headers)
                response = urlopen(req)
                normals_page = response.read()
                response.close()
                try:
                    normals_file_result = json.dumps(
                        {
                            "timestamp": int(time.time()),
                            "normals": json.loads(normals_page),
                        }
                    )
                except:
                    normals_file_result = json.dumps(
                        {
                            "timestamp": int(time.time()),
                            "normals": json.loads(normals_page.decode("utf-8")),
                        }
                    )

                try:
                    with open(normals_file, "wb+") as file:
                        try:
                            # Python 2/3
                            file.write(normals_file_result.encode("utf-8"))
                        except:
                            # Catch errors caused by ASCII characters in Python2
                            file.write(normals_file_result)
                        loginf ("New normals file downloaded to %s" %
                                normals_file)
                except IOError as e:
                    raise Warning(
                        "Error writing normals info to %s. Reason: %s"
                        % (normals_file, e)
                    )
            ...
            if forecast_is_stale:
                try:
                    ...
                    if "forecast_dev_file" in self.generator.skin_dict["Extras"]:
                        ...
                        if forecast_provider == "aeris":
                            ...
                            req = Request(metar_current_url, None, headers)
                            response = urlopen(req)
                            metar_page = response.read()
                            response.close()
                            ...
                            response.close()
                            # AQI - From PurpleAir
                            headers = {'X-API-Key': 'DD8E9A61-E02D-11EC-8561-42010A800005'}                            
                            req = Request(aqi_purpleair_url, None, headers)
                            response = urlopen(req)
                            purpleair_page = response.read()
                            response.close()
                            # restore headers
                            headers = {"User-Agent": user_agent}
                            if (
                                    ...
                                    forecast_file_result = json.dumps( 
                                        {
                                            ...
                                            "metar": json.loads(metar_page),
                                            "forecast_24hr": json.loads(forecast_24hr_page),
                                            ...
                                            "purpleair": json.loads(purpleair_page)
                                        }
                                    )
                                except:
                                    forecast_file_result = json.dumps(
                                        {
                                            ...
                                            "current": 
                                                json.loads(
                                                    current_page.decode("utf-8"))
                                            ,
                                            "metar": json.loads(metar_page),
                                            "forecast_24hr": 
                                                json.loads(
                                                    forecast_24hr_page.decode(
                                                        "utf-8")
                                                )
                                            ,
                                            "forecast_3hr": 
                                                json.loads(
                                                    forecast_3hr_page.decode(
                                                        "utf-8")
                                                )
                                            ,
                                            "forecast_1hr": 
                                                json.loads(
                                                    forecast_1hr_page.decode(
                                                        "utf-8")
                                                )
                                            ,
                                            "alerts": 
                                                json.loads(
                                                    alerts_page.decode("utf-8"))
                                            ,
                                            "aqi": json.loads(aqi_page.decode("utf-8")),
                                            "purpleair": json.loads(purpleair_page.decode("utf-8")),
                                        }
                                    )
                            else:
                                try:
                            ...
                            current_page = response.read()
                            response.close()
                            # METAR Current conditions from Aeris
                            req = Request(metar_current_url, None, headers)
                            response = urlopen(req)
                            metar_page = response.read()
                            response.close()
                            ...
                            try:
                                forecast_file_result = json.dumps(
                                    {
                                        "timestamp": int(time.time()),
                                        "meteoblue": json.loads(current_page),
                                        "metar": json.loads(metar_page),
                                        "purpleair": json.loads(purpleair_page),
                                    }
                                )
                                ...
                                forecast_file_result = json.dumps(
                                    {
                                        "timestamp": int(time.time()),
                                        "meteoblue": json.loads(current_page.decode("utf-8")),
                                        "metar": json.loads(metar_page),
                                        "purpleair": json.loads(purpleair_page.decode("utf-8")),
                                    }
                                )
                    ...
                    if forecast_provider == "aeris":
                        ...
                        raise Warning(
                            "Request Error downloading forecast data for MeteoBlue. "
                            "Check the URLs in your configuration and try again. "
                            "You are trying to use URL: %s, METAR_URL: %s, PURPLEAIR_URL: %s,  The error is: %s"
                            % (forecast_url, metar_current_url, aqi_purpleair_url, error)
                        )
                ...
                try:
                    with open(forecast_file, "wb+") as file:
                        ...
                        loginf("New forecast file downloaded to %s" %
                               forecast_file)
                except IOError as e:
                    raise Warning(
            ...
            if forecast_provider == "aeris":
                if (self.generator.skin_dict["Extras"]["forecast_aeris_use_metar"] == "1"):
                    ...
                    try:
                        cloud_cover = "{}%".format(
                        data["current"]["response"][0]["periods"][0]["sky"])
                    except Exception:
                        loginf("No cloud cover data from Aeris weather")
                        cloud_cover = ""
                try:
                    if (
                        len(data["aqi"]["response"]) > 0
                    ):
                        aqi = data["aqi"]["response"][0]["periods"][0]["aqi"]
                        aqi_category = data["aqi"]["response"][0]["periods"][0]["category"]
                        aqi_time = data["aqi"]["response"][0]["periods"][0]["timestamp"]
                        aqi_location = data["aqi"]["response"][0]["place"]["name"].title(
                        )
                    elif (
                        data["aqi"]["error"]["code"] == "warn_no_data"
                    ):
                        aqi = "No Data"
                        aqi_category = ""
                        aqi_time = 0
                ...
                if (self.generator.skin_dict["Extras"]["forecast_aeris_use_metar"] == "1"):
                    if (len(data["current"]["response"]["ob"]) > 0 ):
                        current_obs_summary = aeris_coded_weather( data["current"]["response"]["ob"]["weatherPrimaryCoded"])
                        current_obs_icon = (
                            aeris_icon(data["current"]["response"]["ob"]["icon"]) + ".png"
                        )

                        if forecast_units in ("si", "ca"):
                            if data["current"]["response"]["ob"]["visibilityKM"] is not None:
                                visibility = locale.format(
                                    "%g", data["current"]["response"]["ob"]["visibilityKM"]
                                )
                                visibility_unit = "km"
                            else:
                                visibility = "N/A"
                            ...
                            if data["current"]["response"]["ob"]["visibilityMI"] is not None:
                                visibility = locale.format(
                                    "%g",
                                    float(data["current"]["response"]["ob"]["visibilityMI"]),
                                )
                ...
                # metar = 0 
                    ...
                    try:
                        current_obs_summary = aeris_coded_weather( data["current"]["response"][0]["periods"][0]["weatherPrimaryCoded"])
                        current_obs_icon = ( aeris_icon(data["current"]["response"][0]["periods"][0]["icon"]) + ".png" )
                        if data["current"]["response"][0]["periods"][0]["visibilityMI"] is not None:
                            visibility = locale.format(
                                "%g",
                                float(data["current"]["response"][0]["periods"][0]["visibilityMI"]),
                            )
                            visibility_unit = "miles"
                        else:
                            visibility = "N/A"
                        ...
                        loginf("No conditions data from Aeris weather")
                        current_obs_summary = ""
                        current_obs_icon = ""
                        visibility = "N/A"
                        visibility_unit = ""
            ...
            #     current_obs_summary = data["meteoblue"]["current"]["weather"][0]["description"]
            #     if data["owm"]["current"]["visibility"] is not None:
            #         visibility = locale.format("%g", float(data["owm"]["current"]["visibility"]), )
            #         visibility_unit = "miles"
            #     else:
                # visibility = "N/A"
                # visibility_unit = ""
            ...
            #     current_obs_icon = data["owm"]["current"]["weather"][0]["icon"]+'@2x.png'

            #     try:
            #         cloud_cover = "{}%".format(
            #             float( data["owm"]["current"]["clouds"] ) )
            #     except Exception:
            #         loginf("No cloud cover data from Open Weather Map weather")
                # cloud_cover = ""
            # ==============================================================================
            # National Weather Service (NWS) Forecast Data
            # ==============================================================================
            # Only process if forecast_nws  is enabled
            

                        # Determine if the file exists and get it's modified time, enhanced
            # for 1 hr forecast to load close to the hour
            if os.path.isfile(forecast_nws_file):
                if (int(time.time()) - int(os.path.getmtime(forecast_nws_file))) > int(
                    forecast_stale_timer
                ):
                    forecast_nws_is_stale = True
                else:
                    # catches repeated calls every archive interval (300secs)
                    if (
                        time.strftime("%M") < "05"
                        and int(time.time()) - int(os.path.getmtime(forecast_nws_file))
                    ) > int(300):
                        forecast_nws_is_stale = True
            else:
                # File doesn't exist, download a new copy
            ...
            if forecast_nws_is_stale:
                try:
                    from urllib.request import Request, urlopen
                    user_agent = "Mozilla/5.0 (Macintosh; U; Intel Mac OS X 10_6_4; en-US) AppleWebKit/534.3 (KHTML, like Gecko) Chrome/6.0.472.63 Safari/534.3"
                    headers = {"User-Agent": user_agent}
                    
                    req = Request(forecast_nws_url, None, headers)                           
                    response = urlopen(req)
                    forecast_nws_page = response.read()
                    response.close()
                    try:
                        forecast_nws_file_result = json.dumps(
                            {
                                "nws": json.loads(forecast_nws_page)
                            }
                        )
                    except:
                        forecast_nws_file_result = json.dumps(
                            {
                                "nws": json.loads(forecast_nws_page.decode("utf-8"))
                            }
                        )
                    try:
                        with open(forecast_nws_file, "wb+") as file:
                            try:
                                file.write(forecast_nws_file_result.encode("utf-8"))
                            except:
                                file.write(forecast_nws_file_result)
                            loginf("New NWS Forecast file downloaded to %s" % forecast_nws_file)
                    except IOError as e:
                        raise Warning("Error writing NWS Forecast File to %s. Reason: %s" % (forecast_nws_file, e))
                except Exception as error:
                    raise Warning(
                        "Error downloads NWS Forecast data. "
                        "Check URL"
                        "You are trying to use URL: %s, and the error is: %s" % (forecast_nws_url, error)
                    )
                ...
                if self.generator.skin_dict["Extras"]["earthquake_server"] == "USGS":
                    ...
                    if distance_unit == "km":
                        ...
                        try:
                            ...
                            eqplace = (
                                str(eqdist_miles) + " miles" +
                                eqmatched.group("rest")
                            )
                        except:
                            eqplace = eqdata["features"][0]["properties"]["place"]
                    eqmag = locale.format(
        ...
        search_list_extension = {
            ...
            "noaa_header_html": json.dumps(noaa_header_html),
